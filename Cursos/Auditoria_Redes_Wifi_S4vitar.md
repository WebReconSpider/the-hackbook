...
- [[#55. Ataques a redes ocultas|55. Ataques a redes ocultas]]
- [[#56. Ataques a Datáfonos|56. Ataques a Datáfonos]]
- [[#57. Uso del Reloj DeAuther - DSTIKE|57. Uso del Reloj DeAuther - DSTIKE]]
- [[#58. Ataques por WPS|58. Ataques por WPS]]
- [[#59. Ataque Comunes (Redes WEP)|59. Ataque Comunes (Redes WEP)]]
...
Una de las funciones clave de Pyrit es la capacidad de analizar y validar los handshakes capturados. Un handshake de cuatro vías (4-way handshake) es un intercambio de mensajes entre un cliente y un punto de acceso que ocurre durante el proceso de autenticación WPA/WPA2. La captura de un handshake completo y válido es esencial para poder realizar ataques de fuerza bruta o diccionario offline.

Pyrit puede verificar si un archivo de captura (`.cap` o `.pcap`) contiene un handshake válido. Esto es crucial porque un handshake incompleto o corrupto no servirá para el cracking de contraseñas. El comando `analyze` de Pyrit se utiliza para este propósito:
...
- `-r [archivo.cap]`: Especifica el archivo de captura a analizar.

Pyrit examinará el archivo y reportará si se encontraron handshakes válidos, cuántos, y la información asociada a ellos (ESSID, BSSID, etc.). Si el handshake es válido, se puede proceder a importarlo a la base de datos de Pyrit para su posterior procesamiento o ataque.

### Uso Avanzado y Opciones

Pyrit ofrece varias opciones y comandos avanzados:

- `benchmark`: Mide el rendimiento de cálculo de PMKs en el sistema.
- `create_essid`: Crea un nuevo ESSID en la base de datos.
- `import_passwords`: Importa listas de contraseñas a la base de datos.
- `attack_db` / `attack_passthrough`: Realiza ataques de fuerza bruta o diccionario utilizando la base de datos de PMKs o listas de contraseñas.
- `strip`: Elimina paquetes irrelevantes de un archivo de captura, dejando solo los handshakes.

Pyrit es una herramienta fundamental para optimizar los ataques de cracking de contraseñas WPA/WPA2, especialmente cuando se trabaja con grandes volúmenes de datos o se busca la máxima eficiencia en el proceso.
...
## 54. Uso de hcxpcaptool y obtención de la contraseña

Este video explica cómo procesar las capturas obtenidas con hcxdumptool utilizando hcxpcaptool para extraer los hashes y prepararlos para el cracking con hashcat.

- **hcxpcaptool** permite extraer diferentes tipos de hashes: PMKID, EAPOL, etc.

### Procedimiento del video:

1. **Procesamiento básico de la captura**:
   ```bash
   hcxpcaptool -z [pmkids].16800 -E [essidlist].txt [captura].pcapng
   ```
   - `-z`: Extrae los hashes PMKID en formato hashcat 16800 y los exporta en ese documento.
   - `-E`: Guarda la lista de ESSIDs detectados.

2. **Análisis de la captura**:
   ```bash
   hcxpcaptool -A [info].txt [captura].pcapng
   ```
   - `-A`: Genera un análisis detallado de la captura.

3. **Cracking con hashcat**:
   ```bash
   hashcat -m 16800 [pmkids].16800 -a 0 [wordlist].txt
   ```
 - `-m 16800`: Modo para PMKID.
   - `-m 22000`: Modo para EAPOL (handshake).
   - `-a 0`: Ataque de diccionario.

## 55. Ataques a redes ocultas

Este video desmitifica la seguridad de las redes Wi-Fi con SSID oculto y demuestra cómo descubrir y atacar estas redes.

### Fundamento técnico:
- **¿Qué es una red oculta?**: Es una red que no emite su nombre (SSID) en los paquetes de baliza (beacon frames). Esto evita que aparezca en la lista de redes Wi-Fi disponibles de los dispositivos.
- **Falsa seguridad**: Ocultar el SSID **no es una medida de seguridad efectiva**. El SSID se sigue transmitiendo en otros tipos de paquetes, como las solicitudes y respuestas de sondeo (probe requests/responses) cuando un cliente legítimo intenta conectarse.

### Procedimiento del video:

1.  **Descubrimiento de la red oculta**: Se utiliza `airodump-ng` para escanear el espectro. Las redes ocultas aparecen con un SSID tipo `<length: 0>` o similar, pero sí muestran su BSSID (dirección MAC).
    ```bash
    airodump-ng <interfaz_monitor>
    ```

2.  **Forzar la revelación del SSID**: Para descubrir el nombre, se puede esperar a que un cliente legítimo se conecte. Cuando lo haga, el SSID se transmitirá en texto plano. Para acelerar el proceso, se puede realizar un ataque de desautenticación contra un cliente que ya esté conectado a la red oculta.
    ```bash
    # Primero, identificar un cliente conectado a la red oculta
    airodump-ng --bssid <BSSID_red_oculta> -c <canal> <interfaz_monitor>
    
    # Luego, desautenticar a ese cliente para forzar una reconexión
    aireplay-ng -0 1 -a <BSSID_red_oculta> -c <MAC_cliente> <interfaz_monitor>
    ```

3.  **Ataque**: Una vez que se ha descubierto el SSID, el ataque procede exactamente igual que con una red normal. Se puede capturar el handshake de 4 vías y proceder a crackear la contraseña.

### Conclusiones:
- Ocultar el SSID ofrece una privacidad mínima frente a observadores casuales, pero ninguna seguridad real contra un atacante determinado.
- Puede ser contraproducente, ya que algunos dispositivos pueden tener problemas de conexión o consumir más batería al buscar activamente redes ocultas.
- No debe considerarse como una medida de seguridad, sino como una simple técnica de oscurecimiento.

## 56. Ataques a Datáfonos

Este video explora un caso específico y especializado: la vulneración de redes WiFi utilizadas por datáfonos.

### Fundamento técnico:
- Los datáfonos modernos suelen utilizar conexiones WiFi para comunicarse con los servidores de procesamiento de pagos.
- El objetivo no es comprometer directamente el datáfono, sino la red WiFi a la que se conecta para potencialmente interceptar o manipular el tráfico.

### Procedimiento del video:

1. **Identificación de redes de datáfonos**:
   ```bash
   airodump-ng <interfaz_monitor>
   ```
   - Se buscan redes con nombres característicos de proveedores de TPV o con patrones de tráfico específicos.

2. **Análisis del tráfico**:
   ```bash
   airodump-ng --bssid <BSSID_datáfono> -c <canal> -w captura <interfaz_monitor>
   ```
   - Se captura el tráfico para analizar patrones y protocolos utilizados.

3. **Captura del handshake**:
   ```bash
   aireplay-ng -0 1 -a <BSSID_datáfono> -c <MAC_datáfono> <interfaz_monitor>
   ```
   - Se intenta desautenticar el datáfono para capturar el handshake cuando se reconecte.

4. **Cracking de la contraseña**:
   ```bash
   aircrack-ng -w <wordlist>.txt <captura>.cap
   ```
   - Se utiliza un diccionario especializado con contraseñas comunes en datáfonos.

### Características específicas de los datáfonos:
- **Contraseñas predeterminadas**: Muchos datáfonos utilizan contraseñas predeterminadas o patrones predecibles.
- **Configuración remota**: Algunos modelos permiten configuración remota, lo que puede ser explotado.
- **Tráfico sensible**: El tráfico puede contener información de transacciones, aunque generalmente está cifrado a nivel de aplicación.
- En un contexto real, estos sistemas suelen estar protegidos por múltiples capas de seguridad más allá de la red WiFi.

## 57. Uso del Reloj DeAuther - DSTIKE

Este video presenta un dispositivo: el reloj DSTIKE WiFi Deauther, una herramienta portable para realizar ataques de denegación de servicio a redes WiFi. No requiere modo monitor ni drivers especiales, ya que implementa los ataques a nivel de paquete en el propio hardware.

### Procedimiento del video:

1. **Configuración inicial del dispositivo**:
   - Encender el dispositivo y conectarse a su red WiFi.

2. **Escaneo de redes**:
   - Desde la interfaz, seleccionar "Scan" para detectar redes cercanas.
   - Seleccionar las redes objetivo marcándolas en la lista.

3. **Tipos de ataques mostrados**:
   - **Deauth Attack**: Envía paquetes de desautenticación a todos los clientes de las redes seleccionadas.
     ```
     [Interfaz Web] → Attacks → Deauth → Start
     ```
   
   - **Beacon Spam**: Crea múltiples redes falsas para saturar el espectro.
     ```
     [Interfaz Web] → Attacks → Beacon → Start
     ```
   
   - **Probe Attack**: Envía solicitudes de sondeo para confundir a los puntos de acceso.
     ```
     [Interfaz Web] → Attacks → Probe → Start
     ```

## 58. Ataques por WPS

Este video aborda una de las vulnerabilidades más graves y extendidas en routers domésticos: el **WPS (Wi-Fi Protected Setup)**.

### Breve explicación:
- **WPS** es un estándar diseñado para facilitar la conexión de dispositivos a una red Wi-Fi sin necesidad de introducir la contraseña. Generalmente, implica presionar un botón en el router o introducir un PIN de 8 dígitos.
- **La vulnerabilidad del PIN**: El fallo de diseño reside en cómo se valida el PIN de 8 dígitos. El router lo valida en dos mitades de 4 dígitos cada una. Además, el último dígito es un checksum. Esto reduce drásticamente el número de combinaciones posibles a crackear:
    - Primera mitad: 10^4 = 10,000 combinaciones.
    - Segunda mitad: 10^3 = 1,000 combinaciones.
    - Total: 11,000 combinaciones en lugar de 10^8 (100 millones).
- Esto hace que un ataque de fuerza bruta para adivinar el PIN sea factible en cuestión de horas.

### Procedimiento del video:

1.  **Identificar redes con WPS activado**: Se utiliza `wash`, una herramienta que viene con `reaver`.
    ```bash
    wash -i <interfaz_monitor>
    ```
    - `wash` mostrará las redes cercanas que tienen WPS habilitado.
> [!note] Resultado en un objetivo con  WPS desabilitado
>-----------------------------------------------------------------------
> BSSID               Ch  dBm  WPS  Lck  Vendor    ESSID  
>---------------------------------------------------------------------
>                                             No  

1.  **Lanzar el ataque con `reaver`**:
    ```bash
    reaver -i <interfaz_monitor> -b <BSSID_objetivo> -vv -K 1
    ```
    - `-i`: Interfaz en modo monitor.
    - `-b`: BSSID de la red objetivo.
    - `-vv`: Modo muy verboso para ver el progreso.
    - `-K 1` o `--pixie-dust`: Intenta el ataque Pixie Dust (ver mejoras).

2.  **Ataque Pixie Dust**: Es mejor que el ataque de fuerza bruta original. Explota implementaciones débiles de WPS en algunos routers (como Ralink, Broadcom, Realtek) que no generan nonces de forma segura. Si el router es vulnerable, el PIN se puede obtener en segundos en lugar de horas.

3.  **Uso de herramientas alternativas**:
    ```bash
    bully <interfaz_monitor> -b <BSSID_objetivo> -c <canal> -v 1
    ```
    - Bully es una alternativa a reaver que a veces funciona mejor contra ciertos routers.

### Contramedidas:
- **La contramedida más efectiva es deshabilitar WPS en la configuración del router.** Esta es la recomendación de seguridad universal.
- Si no se puede deshabilitar, asegurarse de que el firmware del router esté actualizado, ya que algunos fabricantes han implementado protecciones contra estos ataques (aunque no siempre son efectivas).
- Cambiar el PIN de WPS por defecto si el router lo permite (muy pocos lo hacen).

## 59. Ataque Comunes (Redes WEP)

### Descripción de WEP (Wired Equivalent Privacy)
WEP fue el primer protocolo de seguridad implementado para redes inalámbricas 802.11, diseñado para proporcionar un nivel de seguridad comparable al de una red cableada. Sin embargo, WEP ha demostrado ser extremadamente vulnerable y obsoleto. Sus debilidades criptográficas permiten que un atacante con las herramientas adecuadas pueda crackear la clave WEP en cuestión de minutos, o incluso segundos, dependiendo del tráfico de la red.

### Vulnerabilidades Clave de WEP
Las principales vulnerabilidades de WEP se derivan de su diseño criptográfico, particularmente el uso del algoritmo RC4 y la forma en que maneja los Vectores de Inicialización:

1.  **IVs Cortos y Reutilizados**: WEP utiliza un IV de 24 bits, que es demasiado corto. Esto significa que los IVs se repiten con frecuencia, especialmente en redes con mucho tráfico. Cuando se reutiliza un IV con la misma clave WEP, se crea una debilidad criptográfica que puede ser explotada.
2.  **IVs Enviados en Claro**: Los IVs se transmiten en texto plano, lo que permite a un atacante recolectarlos fácilmente.
3.  **Debilidades en el Algoritmo RC4**: El algoritmo de cifrado RC4, tal como se implementa en WEP, es susceptible a ataques si se utilizan IVs débiles o repetidos.

### Ataques Comunes a Redes WEP
Los ataques más comunes y efectivos contra redes WEP se basan en la recolección de un número suficiente de IVs para realizar un análisis estadístico y derivar la clave WEP. Las técnicas incluyen:

1.  **Ataque de Recolección de IVs Pasiva**: Un atacante simplemente escucha el tráfico de la red y recolecta IVs. Cuantos más IVs se recolecten, más rápido se podrá crackear la clave. Este es un ataque pasivo y no genera tráfico adicional.

2.  **Ataque de Inyección ARP (ARP Replay Attack)**: Este es el ataque más popular y efectivo contra WEP. Consiste en inyectar paquetes ARP (Address Resolution Protocol) en la red. Los paquetes ARP son pequeños y generan una gran cantidad de tráfico, lo que a su vez produce muchos IVs nuevos. Al reinyectar un paquete ARP capturado, se fuerza al AP a generar nuevas tramas con nuevos IVs, acelerando drásticamente la recolección de datos.
    -   **Funcionamiento**: Se captura un paquete ARP de la red. Luego, se reinyecta este paquete repetidamente al AP. Cada vez que el AP reenvía el paquete, genera un nuevo IV. Esto permite recolectar miles de IVs por segundo.
    -   **Herramienta**: `aireplay-ng`

3.  **Ataque Chopchop**: Este ataque permite descifrar un paquete cifrado sin conocer la clave WEP. Se basa en la manipulación de paquetes y la observación de las respuestas del AP. Una vez que se descifra un paquete, se puede utilizar para generar nuevos IVs y acelerar el cracking.

4.  **Ataque Fragmentation**: Similar al ataque Chopchop, permite obtener un bloque de texto plano de un paquete cifrado. Una vez obtenido, este bloque puede ser utilizado para inyectar paquetes y generar IVs.

### Proceso General de Ataque WEP
1.  **Poner la interfaz en modo monitor**: `airmon-ng start [INTERFACE]`
2.  **Capturar tráfico y recolectar IVs**: `airodump-ng [INTERFACE_MONITOR]`
3.  **Inyectar paquetes (si es necesario)**: Si el tráfico es bajo, usar `aireplay-ng` para inyectar paquetes ARP y acelerar la recolección de IVs.
4.  **Crackear la clave**: Una vez que se han recolectado suficientes IVs (generalmente decenas de miles), `aircrack-ng` puede derivar la clave WEP.
